...
fun printDivisionProcess(lhv: Int, rhv: Int, outputName: String) {
    var pointer = 1                                                 //указатель на разряд слева направо
    var left = 0
    var right = lhv
    val length = length(lhv)

    fun changeLR() {                                                //перебросить цифру из r в l
        val power = 10.0.pow(length - pointer).toInt()
        left = left * 10 + right / power
        right %= power
    ...
    File(outputName).bufferedWriter().use {
        ...
        if (lhv < rhv) {                                            //критический случай
            ...
            it.newLine()
            // дописываю в конец строки результат деления
            subtract()
            it.newLine()
